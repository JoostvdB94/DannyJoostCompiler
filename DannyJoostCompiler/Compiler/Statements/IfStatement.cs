...
namespace DannyJoostCompiler
{
    public class IfStatement : Statement
    {
        ...
        public override DoubleLinkedList Compile(ref LinkedListNode<Token> currentToken)
        {
                ...
                else if (expectation.Level >= ifLevel)
                {
                    if (condition.First == null)
                    {
                        ...
                        while (currentToken.Value.Level > ifLevel)
                        {
                            ...
                            {
                                condition.AddListLast(conditionStatement.Compile(ref currentToken));
                            }
                            currentToken = currentToken.Next;
                        }
                    ...
                    else
                    {
                        ...
                        while (currentToken.Value.Level > ifLevel)
                        {
                            ...
                            if (conditionStatement != null)
                            {
                                body.AddListLast(conditionStatement.Compile(ref currentToken));
                            }
                            currentToken = currentToken.Next;
                        }
            ...
            if (currentToken.Value.Type == TokenEnumeration.ElseIf)
            {
            ...
            if (currentToken.Value.Type == TokenEnumeration.Else)
            {
